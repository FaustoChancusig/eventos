...
import {
  deleteDoc, doc, updateDoc, collection, query, where, getDocs, addDoc, serverTimestamp, arrayUnion, arrayRemove
} from 'firebase/firestore';
import { db } from '../config/firebase';
import {
  Calendar, MapPin, ArrowLeft, Trash2, Users, Send, Check, HelpCircle,
  X, AlignLeft, Clock, Edit, AlertCircle, CheckCircle, XCircle, Info
} from 'lucide-react';
...
export default function EventDetailPage({ event: initialEvent, user, onBack, onEdit }) {
  ...
  const handleChangeStatus = async (newStatus) => {
    ...
    try {
      ...
      const updates = { attendees: [...otherAttendees, myNewEntry] };
      if (newStatus === 'confirmed' || newStatus === 'maybe') {
        updates.guestIds = arrayUnion(user.uid);
      } else if (newStatus === 'declined') {
        updates.guestIds = arrayRemove(user.uid);
      }
      const notifQuery = query(collection(db, 'users', user.uid, 'notifications'), where('eventId', '==', event.id), where('type', '==', 'invitation'));
      const notifDocs = await getDocs(notifQuery);
      notifDocs.forEach(async (docSnap) => { await deleteDoc(doc(db, 'users', user.uid, 'notifications', docSnap.id)); });
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        await addDoc(collection(db, 'users', targetUser.uid, 'notifications'), {
          type: 'invitation', eventId: event.id, eventName: event.name, fromName: user.displayName || 'Un amigo', fromPhoto: user.photoURL || null, status: 'pending', createdAt: serverTimestamp()
        });
        showNotification('success', `¡Invitación enviada a ${guest.name}!`);
      } else { showNotification('info', `${guest.name} aún no usa la App.`); }
    } catch (error) { showNotification('error', 'Error de conexión.'); } finally { setInvitingIndex(null); }
  };
  ...
  const handleAddGuestFromContacts = async () => {
    try {
      ...
      const currentAttendees = event.attendees || [];
      if (currentAttendees.some(a => normalizePhone(a.phone) === normalizedInput)) { showNotification('warning', 'Ya está en la lista'); return; }
      const newGuest = { uid: null, name: displayName, phone: normalizedInput, status: 'pending', addedBy: user.uid, addedAt: new Date().toISOString(), originalPhone: phoneNumber };
      await updateDoc(doc(db, 'events', event.id), { attendees: arrayUnion(newGuest) });
      setEvent({ ...event, attendees: [...currentAttendees, newGuest] });
      showNotification('success', 'Agregado a la lista.');
    } catch (error) { showNotification('error', 'Error al acceder a contactos'); }
  };
  ...
  const renderInfoContent = () => (
    <div className="p-6 space-y-6 text-ink dark:text-slate-100 pb-24">
      <div className="mb-2">
            ...
            return (
              <div key={index} className="flex flex-col items-center shrink-0 relative group">
                <div className={`relative w-16 h-16 p-0.5 rounded-full border-2 transition-all ${guest.status === 'confirmed' ? 'border-green-500' : guest.status === 'declined' ? 'border-red-300' : 'border-orange-100'}`}>
                  ...
                  {needsInvitation && (
                    <button onClick={() => handleInvite(guest, index)} disabled={invitingIndex === index} className="absolute -top-1 -right-1 w-7 h-7 bg-white text-orange-500 rounded-full shadow-md flex items-center justify-center hover:scale-110 transition-transform">
                ...
                <span className="text-xs mt-2 font-medium text-gray-600 dark:text-gray-400 truncate max-w-[64px] text-center">{isMe ? 'Tú' : guest.name.split(' ')[0]}</span>
              </div>
            );
          })}
        </div>
  ...
  return (
    // CLAVE DEL TECLADO: 'h-[100dvh]' hace que el contenedor mida exactamente lo que se ve en pantalla.
    // Al salir el teclado, '100dvh' se reduce, y como usamos flex, el input sube automáticamente.
    <div className="fixed inset-0 w-full h-[100dvh] flex flex-col bg-gray-100 dark:bg-slate-950 font-sans overflow-hidden">
      
      {/* 1. HEADER FIJO */}
      <div className={`relative shrink-0 z-20 ${headerBgClass} transition-all duration-300`} style={headerBgStyle}>
        ...
        <div className="relative z-20 p-4 flex justify-between items-center">
          <button onClick={onBack} className="bg-black/20 p-2 rounded-full text-white hover:bg-black/30 backdrop-blur-sm"><ArrowLeft size={22} /></button>
          <div className="flex items-center gap-2">
            ...
            <div className="px-4 py-2 bg-white/20 rounded-full text-white text-xs font-bold uppercase backdrop-blur-sm">{event.type}</div>
          </div>
        </div>
        <div className="relative z-10 p-8 pt-2 pb-14">
          <h1 className="text-4xl font-black text-white mb-2 leading-tight drop-shadow-md">{event.name}</h1>
          <p className="text-white/90 text-sm font-medium drop-shadow-sm">Organizado por {event.creatorName}</p>
        </div>
      </div>

      {/* 2. CONTENEDOR CURVADO */}
      <div className="flex-1 flex flex-col bg-gray-100 dark:bg-slate-950 relative z-30 -mt-10 rounded-t-[2.5rem] overflow-hidden shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.3)]">
        
        {/* TABS FIJAS */}
        <div className="shrink-0 bg-gray-100 dark:bg-slate-950 pt-4 border-b border-gray-200 dark:border-slate-700 px-6 flex justify-around">
        ...
        {/* ÁREA DE CONTENIDO */}
        <div className="flex-1 relative overflow-hidden bg-gray-100 dark:bg-slate-950">
          ...
          {currentTab === 'gallery' && (
            <div className="absolute inset-0 overflow-y-auto animate-fade-in p-6">
              <EventGallery eventId={event.id} user={user} isConfirmed={isConfirmed || isCreator} />
            </div>
          )}

          {/* CHAT: Layout específico para que se pegue abajo */}
          {currentTab === 'chat' && (
            <div className="absolute inset-0 flex flex-col bg-white dark:bg-slate-900 animate-fade-in z-50">
               {/* Forzamos a EventChat a ser flex-1 y ocupar el 100% de la altura.
                 Al no tener padding ni margin aquí, y usar h-full, se pegará completamente al borde.
               */}
               <EventChat eventId={event.id} user={user} isConfirmed={isConfirmed || isCreator} className="flex-1 h-full w-full" />
            </div>
          )}
