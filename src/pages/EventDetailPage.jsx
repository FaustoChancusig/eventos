...
export default function EventDetailPage({ event, user, onBack, onEdit }) {
  ...
  // 🆕 Para animación tipo "hero" al montar
    ...
    try {
      ...
      const otherAttendees = currentAttendees.filter(a => {
        ...
        const isMeByPhone =
          myRawPhone &&
          guestPhone &&
          (guestPhone === myRawPhone ||
            guestPhone.includes(myRawPhone) ||
            myRawPhone.includes(guestPhone));
  ...
  const handleInvite = async (guest, index) => {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        let whatsappPhone = rawPhone.startsWith('0')
      ...
      return (
        <div className="p-6 space-y-6 animate-fade-in">
              ...
              {event.attendees && event.attendees.length > 0 ? (
                event.attendees.map((guest, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className="flex flex-col items-center shrink-0 snap-start relative group"
                    >
                      <div
                      ...
                      >
                        ...
                        <div
                          className={`absolute bottom-0 right-0 w-6 h-6 rounded-full border-2 border-white flex items-center justify-center text-white text-xs
                           ${
                             guest.status === 'confirmed'
                               ? 'bg-green-500'
                               : guest.status === 'declined'
                      ...
                      <span
                        className={`text-xs mt-2 font-medium truncate max-w-[70px] ${
                          isMe ? themeAccentColor + ' font-bold' : 'text-gray-600'
                        }`}
                      >
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="bg-white p-1 rounded-[2.5rem] shadow-sm border border-gray-50 overflow-hidden relative group cursor-pointer">
              <div className="h-40 bg-blue-50 w-full rounded-[2rem] relative overflow-hidden opacity-80">
                ...
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white/80 backdrop-blur-md p-4 rounded-2xl shadow-sm flex items-center gap-3 border border-white/50 z-10">
                  <div
                    className={`p-2 rounded-full ${themeBgColor} ${themeAccentColor} shrink-0`}
                  >
                    <MapPin size={20} />
                  </div>
                  <p className="font-bold text-gray-800 line-clamp-2 pr-2">
                    {event.locationName || 'Sin ubicación definida'}
                  </p>
                </div>
              </div>
            ...
            {/* Panel de estado (invitado) */}
            {!isCreator && (
              <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border-2 border-orange-100 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-orange-50 rounded-bl-full opacity-50 pointer-events-none"></div>
                <h3 className="font-bold text-orange-900 text-lg mb-4 text-center relative z-10">
                  ¿Asistirás al evento?
                </h3>
                <div className="grid grid-cols-3 gap-3 relative z-10">
                  <button
                    onClick={() => handleChangeStatus('confirmed')}
                    className={`flex flex-col items-center justify-center p-4 rounded-2xl text-sm font-bold transition active:scale-95 ${
                  ...
                  >
                    <div
                      className={`mb-2 p-2 rounded-full ${
                        currentStatus === 'confirmed' ? 'bg-white/20' : 'bg-white'
                      }`}
                    >
                      <Check size={24} />
                    </div>
                  ...
                  >
                    <div
                      className={`mb-2 p-2 rounded-full ${
                        currentStatus === 'maybe' ? 'bg-white/20' : 'bg-white'
                      }`}
                    >
                      <HelpCircle size={24} />
                    </div>
                  ...
                  >
                    <div
                      className={`mb-2 p-2 rounded-full ${
                        currentStatus === 'declined' ? 'bg-white/20' : 'bg-white'
                      }`}
                    >
                      <X size={24} />
                    </div>
            ...
            {isCreator && (
              <div className="grid grid-cols-2 gap-3 mt-4">
                <button
                  ...
                  className="bg-orange-50 p-4 rounded-[2.5rem] shadow-sm border border-orange-100 flex items-center justify-center gap-3 text-orange-600 font-bold active:scale-95 transition hover:bg-orange-100"
                >
                  <Edit size={22} /> Editar
                </button>
                <button
                  onClick={handleDelete}
                  className="bg-red-50 p-4 rounded-[2.5rem] shadow-sm border border-red-100 flex items-center justify-center gap-3 text-red-600 font-bold active:scale-95 transition hover:bg-red-100"
                >
    ...
    <div className="flex flex-col h-screen bg-gray-100 animate-fade-in font-sans overflow-hidden">
      ...
      >
        ...
        <div className="absolute top-0 left-0 w-full p-4 flex justify-between items-center z-20">
          <button
            onClick={onBack}
            className="bg-black/20 backdrop-blur-md p-2 rounded-full text-white hover:bg-black/30 transition active:scale-95"
          >
            <ArrowLeft size={24} />
          </button>
        ...
        <div className="absolute bottom-0 left-0 w-full p-8 bg-gradient-to-t from-black/60 via-black/30 to-transparent pt-32 z-10">
          <h1 className="text-4xl font-black text-white leading-none mb-2 drop-shadow-lg">
            {event.name}
          </h1>
